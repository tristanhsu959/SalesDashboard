...
trait PosTrait
{
	 ...
	 * @return: array
	 */
	public function getStoreList($brand, $userAreaIds, $type, $name)
	{
		...
		$result = $db->table('SHOP00 as a')
			...
			->when(! empty($type), function ($db) use ($type) {
				$db->whereIn('a.shop_kind', $type);
			})
			->when(! empty($name), function ($db) use ($name) {
				$db->WhereAny(['a.SHOP_NAME'], 'like', "%{$name}%");
			})
	 ...
	 * @return: array
	 */
	public function getHptransStoreList($brand, $userAreaIds, $type, $name)
	{
		$configCode = $brand->code();
		$excepts = config("web.sales.shop.except.{$configCode}");
